...
class Materias(models.Model):
    ...
    dias_json = models.TextField(null=True, blank=True)  # JSON array of days
    horario_inicio = models.TimeField(null=True, blank=True)
    horario_fin = models.TimeField(null=True, blank=True)
    salon = models.CharField(max_length=255, null=True, blank=True)
    programa_educativo = models.TextField(null=True, blank=True)  # JSON array
    maestro = models.ForeignKey(Maestros, on_delete=models.CASCADE, null=True, blank=True)
    creditos = models.IntegerField(null=True, blank=True)
    creation = models.DateTimeField(auto_now_add=True, null=True, blank=True)
    update = models.DateTimeField(null=True, blank=True)

    def __str__(self):
        return f"{self.nrc} - {self.nombre}" if self.nrc else self.nombre
